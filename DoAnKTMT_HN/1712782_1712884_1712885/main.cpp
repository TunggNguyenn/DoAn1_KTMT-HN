...
//Hàm tính toán phép tính trên dòng và xuất kết quả vào outFile theo dòng tương ứng
void CalculateInLine(ofstream &outFile, DataFile datLine, string type)
{
	if (type == "1")
	{

		if (datLine.stringNum == 3)	//yêu cầu chuyển đổi từ 1 hệ này sang 1 hệ khác	
		{
			if (datLine.strLine[0] == "10")
			{
				if (datLine.strLine[1] == "2")	//chuyển đổi từ 10 -> 2
				{
...
void OpenFile(string inName, string outName, string type)
{
	...
	while (!inFile.eof())
	{
		...
		for (i = 0; i < dataFile.stringNum; i++)
		{
			cout << dataFile.strLine[i] << " ";
		}
		cout << endl;

		CalculateInLine(outFile, dataFile, "2");
	}
...
int main()
{
	...
	OpenFile("InputQfloat.txt", "OutputQfloat.txt", "2");

	//Qfloat t = DecToBin("0.0000002");
	//PrintQfloat(t);
	//cout << endl;
